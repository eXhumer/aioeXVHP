...
from aioexvhp.model import StreamffUploadData


async def upload_to_streamff(video_path: Path):
    async with Client() as aioexvhp:
        video = await aioexvhp.upload_to_streamff(
            StreamffUploadData(
...
async def main():
    ...
    await upload_to_streamff(video_path)
