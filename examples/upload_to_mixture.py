...
from aioexvhp.model import MixtureUploadData


async def upload_to_mixture(video_path: Path):
    async with Client() as aioexvhp:
        video = await aioexvhp.upload_to_mixture(
            MixtureUploadData(
...
async def main():
    ...
    await upload_to_mixture(video_path)
